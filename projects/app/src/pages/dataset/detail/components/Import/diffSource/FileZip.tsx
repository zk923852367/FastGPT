...
import { postWordCollection, postZipCollection } from '@/web/core/dataset/api';
...
const FileZip = ({ activeStep, goToNext }: ImportDataComponentProps) => {
  return (
    <>
...
export default React.memo(FileZip);
    ...
    mutationFn: async (files: SelectFileItemType[]) => {
      {
        for await (const selectFile of files) {
          ...
          const data = await postZipCollection(formData);
